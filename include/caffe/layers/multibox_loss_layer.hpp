...
class MultiBoxLossLayer : public LossLayer<Dtype> {
 ...
 protected:
  ...
  shared_ptr<Layer<Dtype> > conf_loss_layer_;
    shared_ptr<Layer<Dtype> > loc1_loss_layer_;
  ...
  vector<Blob<Dtype>*> loc1_bottom_vec_;
  vector<Blob<Dtype>*> loc1_top_vec_;
  Blob<Dtype> loc1_pred_;
  Blob<Dtype> loc1_gt_;
  Blob<Dtype> loc1_loss_;
  int bottomindex[5];
  int bottomsize;
  ...
  AnnotatedDatum_AnnotationType annotation_type_;
};
