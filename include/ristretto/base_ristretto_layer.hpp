...
class BaseRistrettoLayer{
 ...
 protected:
  ...
  void Trim2FixedPoint_cpu(Dtype* data, Dtype* tmp_data, const int cnt, const int bit_width,
    const int rounding, int fl);
  ...
  vector<shared_ptr<Blob<Dtype> > > weights_quantized_;
    vector<shared_ptr<Blob<Dtype> > > weights_quantized_mean_;
    vector<shared_ptr<Blob<Dtype> > > weights_quantized_variance_;
};
