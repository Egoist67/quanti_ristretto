	...
	for L in range(len(layers)):
		...
		if (layer.type == 'Convolution'):
						...
						for q in range(weight_data.shape[3]):
							...
							if rounding == 0:
								if weight_data[i][j][p][q] > 0:
									weight_data[i][j][p][q] = math.ceil(weight_data[i][j][p][q]);
								else:
									weight_data[i][j][p][q] = math.floor(weight_data[i][j][p][q]);
							else:
								weight_data[i][j][p][q] = math.floor(weight_data[i][j][p][q] + RandUniform_cpu());
							weight_data[i][j][p][q] *= pow(2, -fl_conv_params)
			...
			for i in range(bias_data.shape[0]):
				...
				if rounding == 0:
					if bias_data[i] > 0:
						bias_data[i] = math.ceil(bias_data[i])
					else:
						bias_data[i] = math.floor(bias_data[i]);
				else:
					bias_data[i] = math.floor(bias_data[i] + RandUniform_cpu());
				bias_data[i] *= pow(2, -fl_conv_params)
		...
		if (layer.type == 'Scale'):
			...
			for i in range(weight_data.shape[0]):
				...
				if rounding == 0:
					if weight_data[i] > 0:
						weight_data[i] = math.ceil(weight_data[i])
					else:
						weight_data[i] = math.floor(weight_data[i]);
				else:
					weight_data[i] = math.floor(weight_data[i] + RandUniform_cpu());
				weight_data[i] *= pow(2, -fl_scale_params)
				...
				for j in range(weight_data.shape[1]):
					...
					if rounding == 0:
						if weight_data[i][j] > 0:
							weight_data[i][j] = math.ceil(weight_data[i][j])
						else:
							weight_data[i][j] = math.floor(weight_data[i][j]);
					else:
						weight_data[i][j] = math.floor(weight_data[i][j] + RandUniform_cpu());
					weight_data[i][j] *= pow(2, -fl_fc_params)
