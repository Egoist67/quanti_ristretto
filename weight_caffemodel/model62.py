...
if __name__ == "__main__":
	...
	for layer in layers:
		if (layer.type == 'BatchNorm'):
			...
			shape2 = len(net.params[layer.name][1].data.shape)
			print '********',layer.name,shape,shape2
			print 'weight.shape: ',weight_data.shape
			print 'bias.shape: ',bias_data.shape
					...
					for j in range(weight_data.shape[1]):
						for p in range(weight_data.shape[2]):
							for q in range(weight_data.shape[3]):
								...
								#print 'bias_data: ',bias_data[i][j][p][q]
								rounding = layer.quantization_param.rounding_scheme;
								if rounding == 0:
									#print 'rounding 0'
