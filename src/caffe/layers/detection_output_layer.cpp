...
void DetectionOutputLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  use_loc1loss_ = detection_output_param.use_loc1loss();
    if (bottomsize==3)
  {
    for (int i = 0; i < 4; ++i)
    {
  ...
  conf_permute_.ReshapeLike(*(bottom[bottomindex[2]]));
}
...
void DetectionOutputLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  CHECK_EQ(bottom[0]->num(), bottom[bottomindex[2]]->num());
  ...
  if (conf_permute_.num() != bottom[bottomindex[2]]->num() ||
      conf_permute_.count(1) != bottom[bottomindex[2]]->count(1)) {
    conf_permute_.ReshapeLike(*(bottom[bottomindex[2]]));
  }
  ...
  CHECK_EQ(num_priors_ * num_classes_, bottom[bottomindex[2]]->channels())
  ...
  if (annotation_type_ == AnnotatedDatum_AnnotationType_BBOX) {
    ...
    // Each row is a 16 dimension vector, which stores
    // [image_id, label, confidence, xmin, ymin, xmax, ymax, null, x_top_left, ..., y_bottom_left]
    top_shape.push_back(16);
  }else if(annotation_type_ == AnnotatedDatum_AnnotationType_OFFSET_PRETRAIN||annotation_type_ == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING){
...
void DetectionOutputLayer<Dtype>::Forward_cpu(
  ...
  if (bottomsize==3)
  {
      GetLocPredictions(loc_data, num, num_priors_, num_loc_classes_,
                    share_location_, &all_loc_preds, annotation_type_);
  }else if (bottomsize==4)
  {
      GetLocPredictions(loc_data,loc1_data, num, num_priors_, num_loc_classes_,
                    share_location_, &all_loc_preds,annotation_type_);
  }
  ...
  DecodeBBoxesAll(all_loc_preds, prior_bboxes, prior_variances, num,
                  ...
                  &all_decode_bboxes, annotation_type_);
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (int c = 0; c < num_classes_; ++c) {
      ...
      const vector<float>& bg_scores = conf_scores.find(0)->second;
      const vector<float>& fg_scores = conf_scores.find(c)->second;
      vector<float> subtraction_scores;
      for (int s = 0; s < fg_scores.size(); s++) {
        subtraction_scores.push_back(fg_scores[s] - bg_scores[s]);
      }
      const vector<float>& scores = subtraction_scores;
    ...
    if (keep_top_k_ > -1 && num_det > keep_top_k_) {
      ...
      for (map<int, vector<int> >::iterator it = indices.begin();
        ...
        const vector<float>& bg_scores = conf_scores.find(0)->second;
        const vector<float>& fg_scores = conf_scores.find(label)->second;
        vector<float> subtraction_scores;
        for (int s = 0; s < fg_scores.size(); s++) {
          subtraction_scores.push_back(fg_scores[s] - bg_scores[s]);
        }
    ...
    for (map<int, vector<int> >::iterator it = all_indices[i].begin();
      ...
      for (int j = 0; j < indices.size(); ++j) {
        ...
        if (annotation_type_ == AnnotatedDatum_AnnotationType_BBOX) {
          ...
          top_data[count * 16] = i;
          top_data[count * 16 + 1] = label;
          top_data[count * 16 + 2] = scores[idx];
          top_data[count * 16 + 3] = bbox.xmin();
          top_data[count * 16 + 4] = bbox.ymin();
          top_data[count * 16 + 5] = bbox.xmax();
          top_data[count * 16 + 6] = bbox.ymax();

          top_data[count * 16 + 8] = bbox.x_top_left();
          top_data[count * 16 + 9] = bbox.y_top_left();
          top_data[count * 16 + 10] = bbox.x_top_right();
          top_data[count * 16 + 11] = bbox.y_top_right();
          top_data[count * 16 + 12] = bbox.x_bottom_right();
          top_data[count * 16 + 13] = bbox.y_bottom_right();
          top_data[count * 16 + 14] = bbox.x_bottom_left();
          top_data[count * 16 + 15] = bbox.y_bottom_left();
        }else{
          top_data[count * 11] = i;
          top_data[count * 11 + 1] = label;
          top_data[count * 11 + 2] = scores[idx];
          top_data[count * 11 + 3] = bbox.xmin();
          top_data[count * 11 + 4] = bbox.ymin();
          top_data[count * 11 + 5] = bbox.xmax();
          top_data[count * 11 + 6] = bbox.ymax();
          if (annotation_type_ == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING)
          {
            top_data[count * 11 + 7] = bbox.x_offset();
            top_data[count * 11 + 8] = bbox.y_offset();
            top_data[count * 11 + 9] = bbox.width_ratio();
            top_data[count * 11 + 10] = bbox.height_ratio();
          }
        }
        if (need_save_) {
          ...
          float score;
          if (annotation_type_ == AnnotatedDatum_AnnotationType_BBOX) {
            score = top_data[count * 7 + 2];
          } else if (annotation_type_ == AnnotatedDatum_AnnotationType_FOURPOINTS) {
            score = top_data[count * 16 + 2];
          }else if (annotation_type_ == AnnotatedDatum_AnnotationType_OFFSET_PRETRAIN||annotation_type_ == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING){
            score = top_data[count * 11 + 2];
          }
  ...
  if (visualize_) {
    ...
    this->data_transformer_->TransformInv(bottom[bottomindex[3]], &cv_imgs);
