...
void PriorBoxLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  // cout << "min_size_size: " << prior_box_param.min_size_size() << endl;
  ...
  // cout << "aspect_ratios_: " << aspect_ratios_.size() << endl;
  // cout << "min_sizes_: " << min_sizes_.size() << endl;
  // cout << "num_priors_: " << num_priors_ << endl;
  // cout << "max_size_size: " << prior_box_param.max_size_size() << endl;
...
void PriorBoxLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  // LOG(INFO) << "layer_width: " << layer_width;
  // LOG(INFO) << "layer_height: " << layer_height;
    ...
    for (int w = 0; w < layer_width; ++w) {
      ...
      for (int s = 0; s < min_sizes_.size(); ++s) {
        int min_size_ = min_sizes_[s];
	      // cout << "min_size_: " << min_size_ << endl;
        // first prior: aspect_ratio = 1, size = min_size
        box_width = box_height = min_size_;
        // xmin
        top_data[idx++] = (center_x - box_width / 2.) / img_width;
	      //cout << "xmin: " << top_data[idx] << endl;
        // ymin
        top_data[idx++] = (center_y - box_height / 2.) / img_height;
	      //cout << "ymin: " << top_data[idx] << endl;
        // xmax
        top_data[idx++] = (center_x + box_width / 2.) / img_width;
	      //cout << "xmax: " << top_data[idx] << endl;
        // ymax
        top_data[idx++] = (center_y + box_height / 2.) / img_height;
	      //cout << "ymax: " << top_data[idx] << endl;
      ...
      for (int w = 0; w < layer_width; ++w) {
        for (int i = 0; i < num_priors_; ++i) {
          for (int j = 0; j < 4; ++j) {
            ...
            // LOG(INFO) << variance_[j];
            // cout << top_data[count] << endl;
            ++count;
          }
        }
      }
