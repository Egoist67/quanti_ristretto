  ...
  if (this->output_labels_) {
    ...
    if (has_anno_type_) {
      ...
      if (anno_type_ == AnnotatedDatum_AnnotationType_BBOX) {
        ...
        label_shape[3] = 8;
      } else if (anno_type_ == AnnotatedDatum_AnnotationType_FOURPOINTS) {
        // Since the number of bboxes can be different for each image,
        // we store the bbox information in a specific format. In specific:
        // All bboxes are stored in one spatial plane (num and channels are 1)
        // And each row contains one and only one box in the following format:
        // [item_id, group_label, instance_id, xmin, ymin, xmax, ymax, diff, x_top_left, ..., y_bottom_left]
        // Note: Refer to caffe.proto for details about group_label and
        // instance_id.
        for (int g = 0; g < anno_datum.annotation_group_size(); ++g) {
          num_bboxes += anno_datum.annotation_group(g).annotation_size();
        ...
        label_shape[2] = std::max(num_bboxes, 1);
        label_shape[3] = 16;
      } else if (anno_type_ == AnnotatedDatum_AnnotationType_OFFSET_PRETRAIN || anno_type_ == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING) {
        // Since the number of bboxes can be different for each image,
        // we store the bbox information in a specific format. In specific:
        // All bboxes are stored in one spatial plane (num and channels are 1)
        // And each row contains one and only one box in the following format:
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    if (transform_param.has_distort_param()) {
      ...
      if (transform_param.has_expand_param()) {
        ...
        // LOG(INFO)<<"don't has_distort_param has_expand_param";
        expand_datum = &anno_datum;
      }
    }
    AnnotatedDatum* sampled_datum = NULL;
    bool has_sampled = false;
    if (batch_samplers_.size() > 1) {
      ...
      // LOG(INFO)<<"batch_samplers_.size()=1";
      sampled_datum = expand_datum;
    }
    ...
    if (transform_param.has_resize_param()) {
      ...
      // LOG(INFO)<<"not transform_param.has_resize_param()";
      CHECK(std::equal(top_shape.begin() + 1, top_shape.begin() + 4,
            shape.begin() + 1));
    }
    ...
    if (this->output_labels_) {
      if (has_anno_type_) {
        ...
        if (anno_type_ == AnnotatedDatum_AnnotationType_BBOX || anno_type_ == AnnotatedDatum_AnnotationType_FOURPOINTS
           || anno_type_ == AnnotatedDatum_AnnotationType_OFFSET_PRETRAIN  || anno_type_ == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING) {
        ...
        for (int item_id = 0; item_id < batch_size; ++item_id) {
          ...
          for (int g = 0; g < anno_vec.size(); ++g) {
            ...
            for (int a = 0; a < anno_group.annotation_size(); ++a) {
              ...
              top_label[idx++] = bbox.ymax();
              top_label[idx++] = bbox.difficult();
              top_label[idx++] = bbox.x_top_left();
              top_label[idx++] = bbox.y_top_left();
              top_label[idx++] = bbox.x_top_right();
              top_label[idx++] = bbox.y_top_right();
              top_label[idx++] = bbox.x_bottom_right();
              top_label[idx++] = bbox.y_bottom_right();
              top_label[idx++] = bbox.x_bottom_left();
              top_label[idx++] = bbox.y_bottom_left();
            }
          }
        }
      }
        ...
        for (int item_id = 0; item_id < batch_size; ++item_id) {
          ...
          for (int g = 0; g < anno_vec.size(); ++g) {
            ...
            for (int a = 0; a < anno_group.annotation_size(); ++a) {
              ...
              top_label[idx++] = bbox.ymax();
              top_label[idx++] = bbox.difficult();
              top_label[idx++] = bbox.x_offset();
              top_label[idx++] = bbox.y_offset();
              top_label[idx++] = bbox.width_ratio();
              top_label[idx++] = bbox.height_ratio();
              // LOG(INFO) << "set_x_offset: " << bbox.x_offset();
              // LOG(INFO) << "width_ratio: " << bbox.width_ratio();
