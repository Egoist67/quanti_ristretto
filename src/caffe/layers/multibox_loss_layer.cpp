...
void MultiBoxLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  LOG(INFO) << "bottomsize"<<bottomsize;
  LossLayer<Dtype>::LayerSetUp(bottom, top);
  const MultiBoxLossParameter& multibox_loss_param =
      this->layer_param_.multibox_loss_param();
  multibox_loss_param_ = this->layer_param_.multibox_loss_param();
  use_loc1loss_ = multibox_loss_param.use_loc1loss();
  if (this->layer_param_.propagate_down_size() == 0) {
    LOG(INFO)<<"majiawei+++";
    this->layer_param_.add_propagate_down(true);
    if (bottomsize==5)
    {
      LOG(INFO)<<"use_loc1loss_"<<use_loc1loss_;
      this->layer_param_.add_propagate_down(use_loc1loss_);
    }
    this->layer_param_.add_propagate_down(true);
  ...
  if (bottomsize==4)
  {
    ...
    if (bottom[0]->channels() == bottom[2]->height()) {
        LOG(INFO) << "Annotation BBOX";
        annotation_type_ = AnnotatedDatum_AnnotationType_BBOX;
    } else if (bottom[0]->channels() / bottom[2]->height() == 2) {
        LOG(INFO) << "Annotation FOURPOINTS";
        annotation_type_ = AnnotatedDatum_AnnotationType_FOURPOINTS;
    } else {
        LOG(FATAL) << "Unknown annotation type!";
    }
  }else if (bottomsize==5)
  {
    for (int i = 0; i < 5; ++i)
    {
      bottomindex[i] = i;
    ...
    if (use_loc1loss_)
    {
      LOG(INFO) << "Annotation AnnotatedDatum_AnnotationType_OFFSET_FINETUNING";
      annotation_type_ = AnnotatedDatum_AnnotationType_OFFSET_FINETUNING;
    }else{
      annotation_type_ = AnnotatedDatum_AnnotationType_OFFSET_PRETRAIN;
    }
  }
  // LOG(INFO) << "bottomindex[2]"<<bottomindex[2];


  num_ = bottom[0]->num();
  // LOG(INFO) << "bottomindex[3]"<<bottomindex[3];
  num_priors_ = bottom[bottomindex[3]]->height() / 4;
  ...
  if (annotation_type_ == AnnotatedDatum_AnnotationType_FOURPOINTS ) {
    loc_shape.push_back(8);
  } else {
  ...
  // Set up localization1 loss layer.
  if (annotation_type_ == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING||annotation_type_ == AnnotatedDatum_AnnotationType_OFFSET_PRETRAIN)
  {
    // fake shape.
    vector<int> loc1_shape(1, 1);
    loc1_shape.push_back(4);
    loc1_pred_.Reshape(loc1_shape);
    loc1_gt_.Reshape(loc1_shape);
    loc1_bottom_vec_.push_back(&loc1_pred_);
    loc1_bottom_vec_.push_back(&loc1_gt_);
    loc1_loss_.Reshape(loss_shape);
    loc1_top_vec_.push_back(&loc1_loss_);
    if (loc_loss_type_ == MultiBoxLossParameter_LocLossType_L2) {
      LayerParameter layer_param1;
      layer_param1.set_name(this->layer_param_.name() + "_l2_loc1");
      layer_param1.set_type("EuclideanLoss");
      layer_param1.add_loss_weight(loc_weight_);
      loc1_loss_layer_ = LayerRegistry<Dtype>::CreateLayer(layer_param1);
      loc1_loss_layer_->SetUp(loc1_bottom_vec_, loc1_top_vec_);
    } else if (loc_loss_type_ == MultiBoxLossParameter_LocLossType_SMOOTH_L1) {
      LayerParameter layer_param1;
      layer_param1.set_name(this->layer_param_.name() + "_smooth_L1_loc1");
      layer_param1.set_type("SmoothL1Loss");
      layer_param1.add_loss_weight(loc_weight_);
      loc1_loss_layer_ = LayerRegistry<Dtype>::CreateLayer(layer_param1);
      loc1_loss_layer_->SetUp(loc1_bottom_vec_, loc1_top_vec_);
    } else {
...
void MultiBoxLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if (bottomsize==4)
  {
    ...
    if (annotation_type_ == AnnotatedDatum_AnnotationType_BBOX) {
      CHECK_EQ(num_priors_ * loc_classes_ * 4, bottom[0]->channels())
        << "Number of priors must match number of location predictions.";
    } else if (annotation_type_ == AnnotatedDatum_AnnotationType_FOURPOINTS) {
      CHECK_EQ(num_priors_ * loc_classes_ * 8, bottom[0]->channels())
        << "Number of priors must match number of location predictions.";
    }
...
void MultiBoxLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  // LOG(INFO) << "annotation_type_"<<annotation_type_==AnnotatedDatum_AnnotationType_OFFSET_FINETUNING;
  const Dtype* loc_data;
  const Dtype* loc1_data;
  const Dtype* conf_data;
  const Dtype* prior_data;
  const Dtype* gt_data;
  if (bottomsize == 5)
  {
    loc_data = bottom[0]->cpu_data();
    loc1_data = bottom[1]->cpu_data();
    conf_data = bottom[2]->cpu_data();
    prior_data = bottom[3]->cpu_data();
    gt_data = bottom[4]->cpu_data();
  }else if (bottomsize == 4)
  {
    loc_data = bottom[0]->cpu_data();
    conf_data = bottom[1]->cpu_data();
    prior_data = bottom[2]->cpu_data();
    gt_data = bottom[3]->cpu_data();
  }

  // Retrieve all ground truth.
  map<int, vector<NormalizedBBox> > all_gt_bboxes;
  GetGroundTruth(gt_data, num_gt_, background_label_id_, use_difficult_gt_,
                 &all_gt_bboxes, annotation_type_);
  ...
  if (bottomsize==4)
  {
    GetLocPredictions(loc_data, num_, num_priors_, loc_classes_, share_location_,
                      &all_loc_preds, annotation_type_);
  }else if (bottomsize==5)
  {
    GetLocPredictions(loc_data, loc1_data, num_, num_priors_, loc_classes_, share_location_,
                      &all_loc_preds, annotation_type_);
  }
  ...
  FindMatches(all_loc_preds, all_gt_bboxes, prior_bboxes, prior_variances,
              multibox_loss_param_, &all_match_overlaps, &all_match_indices_, annotation_type_);

  num_matches_ = 0;
  int num_negs = 0;
    // Sample hard negative (and positive) examples based on mining type.
  MineHardExamples(*bottom[bottomindex[2]], all_loc_preds, all_gt_bboxes, prior_bboxes,
               prior_variances, all_match_overlaps, multibox_loss_param_,
               &num_matches_, &num_negs, &all_match_indices_,
               &all_neg_indices_, annotation_type_);
  if (num_matches_ >= 1) {
    ...
    if (bottomsize==4)
    {
      ...
      EncodeLocPrediction(all_loc_preds, all_gt_bboxes, all_match_indices_,
                          ...
                          loc_pred_data, loc_gt_data, annotation_type_);
      loc_loss_layer_->Reshape(loc_bottom_vec_, loc_top_vec_);
      loc_loss_layer_->Forward(loc_bottom_vec_, loc_top_vec_);
    }else if (bottomsize==5)
    {
      vector<int> loc_shape(2);
      vector<int> loc1_shape(2);
      loc_shape[0] = 1;
      loc_shape[1] = num_matches_ * 4;
      loc1_shape[0] = 1;
      loc1_shape[1] = num_matches_ * 4;
      loc_pred_.Reshape(loc_shape);
      loc1_pred_.Reshape(loc1_shape);
      loc_gt_.Reshape(loc_shape);
      loc1_gt_.Reshape(loc1_shape);
      Dtype* loc_pred_data = loc_pred_.mutable_cpu_data();
      Dtype* loc_gt_data = loc_gt_.mutable_cpu_data();
      Dtype* loc1_pred_data = loc1_pred_.mutable_cpu_data();
      Dtype* loc1_gt_data = loc1_gt_.mutable_cpu_data();
      EncodeLocPrediction(all_loc_preds, all_gt_bboxes, all_match_indices_,
                          prior_bboxes, prior_variances, multibox_loss_param_,
                          loc_pred_data, loc_gt_data,loc1_pred_data,loc1_gt_data, annotation_type_);
      loc_loss_layer_->Reshape(loc_bottom_vec_, loc_top_vec_);
      loc_loss_layer_->Forward(loc_bottom_vec_, loc_top_vec_);
      loc1_loss_layer_->Reshape(loc1_bottom_vec_, loc1_top_vec_);
      loc1_loss_layer_->Forward(loc1_bottom_vec_, loc1_top_vec_);
    }
    ...
    if (bottomsize==5)
    {
      loc1_loss_.mutable_cpu_data()[0] = 0;
    }
  }
  ...
  if (num_conf_ >= 1) {
    ...
    if (!do_neg_mining_) {
      ...
      CHECK_EQ(conf_pred_.count(), bottom[bottomindex[2]]->count());
      conf_pred_.ShareData(*(bottom[bottomindex[2]]));
  ...
  if (bottomsize==5)
  {
    if (this->layer_param_.propagate_down(1)) {
      // LOG(INFO)<<"GetNormalizer";
      Dtype normalizer = LossLayer<Dtype>::GetNormalizer(
          normalization_, num_, num_priors_, num_matches_);
      top[0]->mutable_cpu_data()[0] +=
          loc_weight_ * loc1_loss_.cpu_data()[0] / normalizer;
    }
  }
  if (this->layer_param_.propagate_down(bottomindex[2])) {
    Dtype normalizer = LossLayer<Dtype>::GetNormalizer(
        normalization_, num_, num_priors_, num_matches_);
...
void MultiBoxLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[bottomindex[3]]) {
    LOG(FATAL) << this->type()
        << " Layer cannot backpropagate to prior inputs.";
  }
  if (propagate_down[bottomindex[4]]) {
    LOG(FATAL) << this->type()
      ...
      for (int i = 0; i < num_; ++i) {
        for (map<int, vector<int> >::iterator it =
          ...
          for (int j = 0; j < match_index.size(); ++j) {
            ...
            if (annotation_type_ == AnnotatedDatum_AnnotationType_BBOX || annotation_type_ == AnnotatedDatum_AnnotationType_OFFSET_PRETRAIN|| annotation_type_ == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING) {
              int start_idx = loc_classes_ * 4 * j + label * 4;
              caffe_copy<Dtype>(4, loc_pred_diff + count * 4,
                              loc_bottom_diff + start_idx);
            } else if (annotation_type_ == AnnotatedDatum_AnnotationType_FOURPOINTS) {
              int start_idx = loc_classes_ * 8 * j + label * 8;
              caffe_copy<Dtype>(8, loc_pred_diff + count * 8,
                              loc_bottom_diff + start_idx);
            }
            ++count;
          }
...
if (bottomsize==5)
{
      // Back propagate on location1 prediction.
  if (propagate_down[1]) {
    // LOG(INFO)<<"majiawei:";
    Dtype* loc1_bottom_diff = bottom[1]->mutable_cpu_diff();
    caffe_set(bottom[1]->count(), Dtype(0), loc1_bottom_diff);
    if (num_matches_ >= 1) {
      vector<bool> loc1_propagate_down;
      // Only back propagate on prediction, not ground truth.
      loc1_propagate_down.push_back(true);
      loc1_propagate_down.push_back(false);
      loc1_loss_layer_->Backward(loc1_top_vec_, loc1_propagate_down,
                                loc1_bottom_vec_);
      // Scale gradient.
      Dtype normalizer = LossLayer<Dtype>::GetNormalizer(
          normalization_, num_, num_priors_, num_matches_);
      Dtype loss_weight = top[0]->cpu_diff()[0] / normalizer;
      caffe_scal(loc1_pred_.count(), loss_weight, loc1_pred_.mutable_cpu_diff());
      // Copy gradient back to bottom[0].
      const Dtype* loc1_pred_diff = loc1_pred_.cpu_diff();
      int count = 0;
      for (int i = 0; i < num_; ++i) {
        for (map<int, vector<int> >::iterator it =
             all_match_indices_[i].begin();
          ...
          for (int j = 0; j < match_index.size(); ++j) {
            ...
            // Copy the diff to the right place.
            int start_idx = loc_classes_ * 4 * j + label * 4;
            caffe_copy<Dtype>(4, loc1_pred_diff + count * 4,
                              loc1_bottom_diff + start_idx);
            ++count;
          }
        }
        loc1_bottom_diff += bottom[1]->offset(1);
      }
    }
  }
  ...
  if (propagate_down[bottomindex[2]]) {
    Dtype* conf_bottom_diff = bottom[bottomindex[2]]->mutable_cpu_diff();
    caffe_set(bottom[bottomindex[2]]->count(), Dtype(0), conf_bottom_diff);
    if (num_conf_ >= 1) {
      ...
      if (do_neg_mining_) {
        ...
        for (int i = 0; i < num_; ++i) {
          ...
          conf_bottom_diff += bottom[bottomindex[2]]->offset(1);
        }
      } else {
        // The diff is already computed and stored.
        bottom[bottomindex[2]]->ShareDiff(conf_pred_);
      }
    }
  }
