...
bool ReadRichImageToAnnotatedDatum(const string& filename,
  ...
  switch (type) {
    ...
    case AnnotatedDatum_AnnotationType_FOURPOINTS:
    case AnnotatedDatum_AnnotationType_OFFSET_PRETRAIN:
    case AnnotatedDatum_AnnotationType_OFFSET_FINETUNING:
...
bool ReadXMLToAnnotatedDatum(const string& labelfile, const int img_height,
    ...
    AnnotatedDatum* anno_datum, const AnnotatedDatum_AnnotationType type) {
  ptree pt;
    ...
    if (v1.first == "object") {
      ...
      BOOST_FOREACH(ptree::value_type &v2, object.get_child("")) {
        ...
        if (v2.first == "name") {
          ...
          int x_top_left = 0, y_top_left = 0, x_top_right = 0, y_top_right = 0, 
          x_bottom_right = 0, y_bottom_right = 0, x_bottom_left = 0, y_bottom_left = 0,
          x_offset = 0, y_offset = 0;
          float width_ratio,height_ratio;
          if (type == AnnotatedDatum_AnnotationType_FOURPOINTS) {
            x_top_left = pt2.get("x_top_left", 0);
            y_top_left = pt2.get("y_top_left", 0);
            x_top_right = pt2.get("x_top_right", 0);
            y_top_right = pt2.get("y_top_right", 0);
            x_bottom_right = pt2.get("x_bottom_right", 0);
            y_bottom_right = pt2.get("y_bottom_right", 0);
            x_bottom_left = pt2.get("x_bottom_left", 0);
            y_bottom_left = pt2.get("y_bottom_left", 0);
          } else if (type == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING || type == AnnotatedDatum_AnnotationType_OFFSET_PRETRAIN)
          {
            x_offset = pt2.get("x_offset", 0);
            y_offset = pt2.get("y_offset", 0);
            width_ratio = pt2.get("width_ratio", 0.0);
            height_ratio = pt2.get("height_ratio", 0.0);
            // LOG(INFO)<<"width_ratio"<<width_ratio<<"height_ratio"<<height_ratio;
          }
          CHECK_NOTNULL(anno);
          ...
          if (type == AnnotatedDatum_AnnotationType_FOURPOINTS) {
            bbox->set_x_top_left(static_cast<float>(x_top_left) / width);
            bbox->set_y_top_left(static_cast<float>(y_top_left) / height);
            bbox->set_x_top_right(static_cast<float>(x_top_right) / width);
            bbox->set_y_top_right(static_cast<float>(y_top_right) / height);
            bbox->set_x_bottom_right(static_cast<float>(x_bottom_right) / width);
            bbox->set_y_bottom_right(static_cast<float>(y_bottom_right) / height);
            bbox->set_x_bottom_left(static_cast<float>(x_bottom_left) / width);
            bbox->set_y_bottom_left(static_cast<float>(y_bottom_left) / height);
          } else if (type == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING || type == AnnotatedDatum_AnnotationType_OFFSET_PRETRAIN)
          {
            bbox->set_x_offset(static_cast<float>(x_offset) / (xmax - xmin) * 2);
            bbox->set_y_offset(static_cast<float>(y_offset) / (ymax - ymin) * 2);
            bbox->set_width_ratio(width_ratio);
            bbox->set_height_ratio(height_ratio);
          }
        }
      }
    }
