  ...
  if (has_resize && resize_param.resize_mode()) {
    ...
    switch (resize_param.resize_mode()) {
      ...
      case ResizeParameter_Resize_mode_FIT_LARGE_SIZE_AND_PAD:
        ...
        ProjectBBox(source_bbox, bbox, &temp_bbox, AnnotatedDatum_AnnotationType_BBOX);
        ClipBBox(temp_bbox, &temp_bbox);
        ScaleBBox(temp_bbox, height, width, out_bbox);
        break;
...
bool ProjectBBox(const NormalizedBBox& src_bbox, const NormalizedBBox& bbox,
                 NormalizedBBox* proj_bbox, AnnotatedDatum_AnnotationType annotation_type) {
  ...
  if (annotation_type == AnnotatedDatum_AnnotationType_FOURPOINTS) {
    proj_bbox->set_x_top_left((bbox.x_top_left() - src_bbox.xmin()) / src_width);
    proj_bbox->set_y_top_left((bbox.y_top_left() - src_bbox.ymin()) / src_height);
    proj_bbox->set_x_top_right((bbox.x_top_right() - src_bbox.xmin()) / src_width);
    proj_bbox->set_y_top_right((bbox.y_top_right() - src_bbox.ymin()) / src_height);
    proj_bbox->set_x_bottom_right((bbox.x_bottom_right() - src_bbox.xmin()) / src_width);
    proj_bbox->set_y_bottom_right((bbox.y_bottom_right() - src_bbox.ymin()) / src_height);
    proj_bbox->set_x_bottom_left((bbox.x_bottom_left() - src_bbox.xmin()) / src_width);
    proj_bbox->set_y_bottom_left((bbox.y_bottom_left() - src_bbox.ymin()) / src_height);
  }
  if (annotation_type == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING)
  {
    proj_bbox->set_x_offset((bbox.x_offset() - src_bbox.x_offset()) / src_width);
    proj_bbox->set_y_offset((bbox.y_offset() - src_bbox.y_offset()) / src_height);
    proj_bbox->set_width_ratio((bbox.width_ratio() - src_bbox.width_ratio()) / src_width);
    proj_bbox->set_height_ratio((bbox.height_ratio() - src_bbox.height_ratio()) / src_height);
  }
...
void EncodeBBox(
    ...
    const NormalizedBBox& bbox, NormalizedBBox* encode_bbox, AnnotatedDatum_AnnotationType annotation_type) {
  if (code_type == PriorBoxParameter_CodeType_CORNER) {
    ...
    float bbox_x_top_left, bbox_y_top_left, bbox_x_top_right, bbox_y_top_right,
    bbox_x_bottom_right, bbox_y_bottom_right, bbox_x_bottom_left, bbox_y_bottom_left;
    float bbox_x_offset,bbox_y_offset;
    float bbox_width_ratio,bbox_height_ratio;
    if (annotation_type == AnnotatedDatum_AnnotationType_FOURPOINTS) {
      bbox_x_top_left = bbox.x_top_left();
      bbox_y_top_left = bbox.y_top_left();
      bbox_x_top_right = bbox.x_top_right();
      bbox_y_top_right = bbox.y_top_right();
      bbox_x_bottom_right = bbox.x_bottom_right();
      bbox_y_bottom_right = bbox.y_bottom_right();
      bbox_x_bottom_left = bbox.x_bottom_left();
      bbox_y_bottom_left = bbox.y_bottom_left();
    }else if (annotation_type == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING)
    {
      bbox_x_offset = bbox.x_offset();
      bbox_y_offset = bbox.y_offset();
      bbox_width_ratio = bbox.width_ratio();
      bbox_height_ratio = bbox.height_ratio();
    }
    if (encode_variance_in_target) {
      ...
      encode_bbox->set_xmax(
          bbox_width / prior_width / prior_variance[2]);
      encode_bbox->set_ymax(
          bbox_height / prior_height / prior_variance[3]);
      if (annotation_type == AnnotatedDatum_AnnotationType_FOURPOINTS) {
        encode_bbox->set_x_top_left(
                  (bbox_x_top_left - prior_center_x) / prior_width / prior_variance[0]);
        encode_bbox->set_y_top_left(
                    (bbox_y_top_left - prior_center_y) / prior_height / prior_variance[0]);
        encode_bbox->set_x_top_right(
                    (bbox_x_top_right - prior_center_x) / prior_width / prior_variance[0]);
        encode_bbox->set_y_top_right(
                    (bbox_y_top_right - prior_center_y) / prior_height / prior_variance[0]);
        encode_bbox->set_x_bottom_right(
                    (bbox_x_bottom_right - prior_center_x) / prior_width / prior_variance[0]);
        encode_bbox->set_y_bottom_right(
                    (bbox_y_bottom_right - prior_center_y) / prior_height / prior_variance[0]);
        encode_bbox->set_x_bottom_left(
                    (bbox_x_bottom_left - prior_center_x) / prior_width / prior_variance[0]);
        encode_bbox->set_y_bottom_left(
                    (bbox_y_bottom_left - prior_center_y) / prior_height / prior_variance[0]);
      }else if (annotation_type == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING)
      {
        encode_bbox->set_x_offset(
          bbox_x_offset / prior_width / prior_variance[0]);
        encode_bbox->set_y_offset(
          bbox_y_offset / prior_height / prior_variance[0]);
        encode_bbox->set_width_ratio(
          bbox_width_ratio / prior_width / prior_variance[0]);
        encode_bbox->set_height_ratio(
          bbox_height_ratio / prior_height / prior_variance[0]);
      }
    }
...
void DecodeBBox(
    ...
    NormalizedBBox* decode_bbox, AnnotatedDatum_AnnotationType annotation_type) {
  if (code_type == PriorBoxParameter_CodeType_CORNER) {
    ...
    float decode_bbox_x_top_left, decode_bbox_y_top_left, decode_bbox_x_top_right, decode_bbox_y_top_right;
    float decode_bbox_x_bottom_right, decode_bbox_y_bottom_right, decode_bbox_x_bottom_left, decode_bbox_y_bottom_left;
    float decode_bbox_x_offset,decode_bbox_y_offset;
    float decode_bbox_width_ratio,decode_bbox_height_ratio;
    if (variance_encoded_in_target) {
      ...
      // variance is encoded in bbox, we need to scale the offset accordingly.
      if (annotation_type == AnnotatedDatum_AnnotationType_BBOX || annotation_type == AnnotatedDatum_AnnotationType_OFFSET_PRETRAIN||annotation_type == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING) {
        
        decode_bbox_center_x =
          prior_variance[0] * bbox.xmin() * prior_width + prior_center_x;
          // LOG(INFO)<<"decode_bbox_center_x"<<decode_bbox_center_x;
      ...
      } else if (annotation_type == AnnotatedDatum_AnnotationType_FOURPOINTS) {
        decode_bbox_x_top_left = prior_variance[0] * bbox.x_top_left() * prior_width + prior_center_x;
        decode_bbox_y_top_left = prior_variance[0] * bbox.y_top_left() * prior_height + prior_center_y;
        decode_bbox_x_top_right = prior_variance[0] * bbox.x_top_right() * prior_width + prior_center_x;
        decode_bbox_y_top_right = prior_variance[0] * bbox.y_top_right() * prior_height + prior_center_y;
        decode_bbox_x_bottom_right = prior_variance[0] * bbox.x_bottom_right() * prior_width + prior_center_x;
        decode_bbox_y_bottom_right = prior_variance[0] * bbox.y_bottom_right() * prior_height + prior_center_y;
        decode_bbox_x_bottom_left = prior_variance[0] * bbox.x_bottom_left() * prior_width + prior_center_x;
        decode_bbox_y_bottom_left = prior_variance[0] * bbox.y_bottom_left() * prior_height + prior_center_y;
      }
      if (annotation_type == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING)
      {
        decode_bbox_x_offset = prior_variance[0] * bbox.x_offset() * prior_width;
        decode_bbox_y_offset = prior_variance[0] * bbox.y_offset() * prior_height;
        decode_bbox_width_ratio = prior_variance[0] * bbox.width_ratio() * prior_width;
        decode_bbox_height_ratio = prior_variance[0] * bbox.height_ratio() * prior_height;
        // LOG(INFO)<<"decode_bbox_y_offset"<<decode_bbox_y_offset;
      }
    }
    if (annotation_type == AnnotatedDatum_AnnotationType_BBOX || annotation_type == AnnotatedDatum_AnnotationType_OFFSET_PRETRAIN||annotation_type == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING) {
      ...
      decode_bbox->set_xmin(min(decode_bbox_x_top_left, decode_bbox_x_bottom_left));
      decode_bbox->set_ymin(min(decode_bbox_y_top_left, decode_bbox_y_top_right));
      decode_bbox->set_xmax(max(decode_bbox_x_bottom_right, decode_bbox_x_top_right));
      decode_bbox->set_ymax(max(decode_bbox_y_bottom_right, decode_bbox_y_bottom_left));
      decode_bbox->set_x_top_left(decode_bbox_x_top_left);
      decode_bbox->set_y_top_left(decode_bbox_y_top_left);
      decode_bbox->set_x_top_right(decode_bbox_x_top_right);
      decode_bbox->set_y_top_right(decode_bbox_y_top_right);
      decode_bbox->set_x_bottom_right(decode_bbox_x_bottom_right);
      decode_bbox->set_y_bottom_right(decode_bbox_y_bottom_right);
      decode_bbox->set_x_bottom_left(decode_bbox_x_bottom_left);
      decode_bbox->set_y_bottom_left(decode_bbox_y_bottom_left);
    }
    if (annotation_type == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING)
    {
      decode_bbox->set_x_offset(decode_bbox_x_offset);
      decode_bbox->set_y_offset(decode_bbox_y_offset);
      decode_bbox->set_width_ratio(decode_bbox_width_ratio);
      decode_bbox->set_height_ratio(decode_bbox_height_ratio);
    }
...
void DecodeBBoxes(
    ...
    vector<NormalizedBBox>* decode_bboxes, AnnotatedDatum_AnnotationType annotation_type) {
  ...
  for (int i = 0; i < num_bboxes; ++i) {
    ...
    DecodeBBox(prior_bboxes[i], prior_variances[i], code_type,
               variance_encoded_in_target, clip_bbox, bboxes[i], &decode_bbox, annotation_type);
...
void DecodeBBoxesAll(const vector<LabelBBox>& all_loc_preds,
    ...
    const CodeType code_type, const bool variance_encoded_in_target,
    const bool clip, vector<LabelBBox>* all_decode_bboxes, AnnotatedDatum_AnnotationType annotation_type) {
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (int c = 0; c < num_loc_classes; ++c) {
      ...
      DecodeBBoxes(prior_bboxes, prior_variances,
                   ...
                   label_loc_preds, &(decode_bboxes[label]), annotation_type);
    }
  }
}
...
void FindMatches(const vector<LabelBBox>& all_loc_preds,
      ...
      vector<map<int, vector<int> > >* all_match_indices, AnnotatedDatum_AnnotationType annotation_type) {
    ...
    if (!use_prior_for_matching) {
      for (int c = 0; c < loc_classes; ++c) {
        ...
        DecodeBBoxes(prior_bboxes, prior_variances,
                     ...
                     all_loc_preds[i].find(label)->second, &loc_bboxes, annotation_type);
...
void MineHardExamples(const Blob<Dtype>& conf_blob,
    ...
    vector<vector<int> >* all_neg_indices, AnnotatedDatum_AnnotationType annotation_type) {
  int num = all_loc_preds.size();
...
// #endif
  ...
  if (mining_type == MultiBoxLossParameter_MiningType_HARD_EXAMPLE) {
    ...
    Blob<Dtype> loc_pred, loc_gt,loc1_pred, loc1_gt;
    if (*num_matches != 0) {
      ...
      if (annotation_type == AnnotatedDatum_AnnotationType_OFFSET_PRETRAIN || annotation_type == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING)
      {
        vector<int> loc1_shape(2, 1);
        loc1_shape[1] = *num_matches * 4;
        loc1_pred.Reshape(loc1_shape);
        loc1_gt.Reshape(loc1_shape);
        Dtype* loc1_pred_data = loc1_pred.mutable_cpu_data();
        Dtype* loc1_gt_data = loc1_gt.mutable_cpu_data();
        EncodeLocPrediction(all_loc_preds, all_gt_bboxes, *all_match_indices,
                          prior_bboxes, prior_variances, multibox_loss_param,
                          loc_pred_data, loc_gt_data,loc1_pred_data, loc1_gt_data, annotation_type);
        LOG(INFO)<<"majiawei1";
      }else{
        LOG(INFO)<<"majiawei";
        EncodeLocPrediction(all_loc_preds, all_gt_bboxes, *all_match_indices,
                    prior_bboxes, prior_variances, multibox_loss_param,
                    loc_pred_data, loc_gt_data, annotation_type);
      }
    }
...
void GetGroundTruth(const Dtype* gt_data, const int num_gt,
      ...
      map<int, vector<NormalizedBBox> >* all_gt_bboxes, AnnotatedDatum_AnnotationType annotation_type) {
  all_gt_bboxes->clear();
  for (int i = 0; i < num_gt; ++i) {
    int start_idx;
    if (annotation_type == AnnotatedDatum_AnnotationType_BBOX) {
      start_idx = i * 8;
    } else if (annotation_type == AnnotatedDatum_AnnotationType_FOURPOINTS) {
      start_idx = i * 16;
    }else if (annotation_type == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING || annotation_type == AnnotatedDatum_AnnotationType_OFFSET_PRETRAIN)
    {
      start_idx = i * 12;
    }
    ...
    if (annotation_type == AnnotatedDatum_AnnotationType_FOURPOINTS) {
      // LOG(INFO) << "gawain111";
      bbox.set_x_top_left(gt_data[start_idx + 8]);
      bbox.set_y_top_left(gt_data[start_idx + 9]);
      bbox.set_x_top_right(gt_data[start_idx + 10]);
      bbox.set_y_top_right(gt_data[start_idx + 11]);
      bbox.set_x_bottom_right(gt_data[start_idx + 12]);
      bbox.set_y_bottom_right(gt_data[start_idx + 13]);
      bbox.set_x_bottom_left(gt_data[start_idx + 14]);
      bbox.set_y_bottom_left(gt_data[start_idx + 15]);
    }
    if (annotation_type == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING)
    {
      // LOG(INFO) << "gt_data[start_idx + 8]"<<gt_data[start_idx + 8];
      bbox.set_x_offset(gt_data[start_idx + 8]);
      bbox.set_y_offset(gt_data[start_idx + 9]);
      bbox.set_width_ratio(gt_data[start_idx + 10]);
      bbox.set_height_ratio(gt_data[start_idx + 11]);
    }
...
void GetGroundTruth(const Dtype* gt_data, const int num_gt,
      const int background_label_id, const bool use_difficult_gt,
      map<int, LabelBBox>* all_gt_bboxes, AnnotatedDatum_AnnotationType annotation_type) {
  all_gt_bboxes->clear();
  for (int i = 0; i < num_gt; ++i) {
    int start_idx;
    if (annotation_type == AnnotatedDatum_AnnotationType_BBOX) {
...
void GetLocPredictions(const Dtype* loc_data, const int num,
      ...
      const bool share_location, vector<LabelBBox>* loc_preds, AnnotatedDatum_AnnotationType annotation_type) {
  loc_preds->clear();
  if (share_location) {
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (int p = 0; p < num_preds_per_class; ++p) {
      int start_idx;
      if (annotation_type == AnnotatedDatum_AnnotationType_BBOX) {
        start_idx = p * num_loc_classes * 4;
      } else if (annotation_type == AnnotatedDatum_AnnotationType_FOURPOINTS) {
        start_idx = p * num_loc_classes * 8;
      }
      for (int c = 0; c < num_loc_classes; ++c) {
        int label = share_location ? -1 : c;
        ...
        if (annotation_type == AnnotatedDatum_AnnotationType_BBOX) {
          ...
          label_bbox[label][p].set_x_top_left(loc_data[start_idx + c * 8]);
          label_bbox[label][p].set_y_top_left(loc_data[start_idx + c * 8 + 1]);
          label_bbox[label][p].set_x_top_right(loc_data[start_idx + c * 8 + 2]);
          label_bbox[label][p].set_y_top_right(loc_data[start_idx + c * 8 + 3]);
          label_bbox[label][p].set_x_bottom_right(loc_data[start_idx + c * 8 + 4]);
          label_bbox[label][p].set_y_bottom_right(loc_data[start_idx + c * 8 + 5]);
          label_bbox[label][p].set_x_bottom_left(loc_data[start_idx + c * 8 + 6]);
          label_bbox[label][p].set_y_bottom_left(loc_data[start_idx + c * 8 + 7]);
        }
      }
    }
    if (annotation_type == AnnotatedDatum_AnnotationType_BBOX) {
...
void GetLocPredictions(const Dtype* loc_data,const Dtype* loc1_data, const int num,
      const int num_preds_per_class, const int num_loc_classes,
      const bool share_location, vector<LabelBBox>* loc_preds,AnnotatedDatum_AnnotationType annotation_type) {
  loc_preds->clear();
  if (share_location) {
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (int p = 0; p < num_preds_per_class; ++p) {
      ...
      if (annotation_type == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING)
      {
        start_idx1 = p * num_loc_classes * 4;
      }
      for (int c = 0; c < num_loc_classes; ++c) {
        int label = share_location ? -1 : c;
        if (label_bbox.find(label) == label_bbox.end()) {
        ...
        if (annotation_type == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING)
        {
          // LOG(INFO)<<"loc1_data[start_idx1 + c * 2]"<<loc1_data[start_idx1 + c * 2];
          label_bbox[label][p].set_x_offset(loc1_data[start_idx1 + c * 4]);
          label_bbox[label][p].set_y_offset(loc1_data[start_idx1 + c * 4 + 1]);
          label_bbox[label][p].set_width_ratio(loc1_data[start_idx1 + c * 4 + 2]);
          label_bbox[label][p].set_height_ratio(loc1_data[start_idx1 + c * 4 + 3]);
        }
      }
    }
    loc_data += num_preds_per_class * num_loc_classes * 4;
    if (annotation_type == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING)
    {
      loc1_data += num_preds_per_class * num_loc_classes * 4;
    }
...
// Explicit initialization.
template void GetLocPredictions(const float* loc_data,const float* loc1_data, const int num,
      const int num_preds_per_class, const int num_loc_classes,
      const bool share_location, vector<LabelBBox>* loc_preds,AnnotatedDatum_AnnotationType annotation_type);
template void GetLocPredictions(const double* loc_data,const double* loc1_data, const int num,
      const int num_preds_per_class, const int num_loc_classes,
      const bool share_location, vector<LabelBBox>* loc_preds,AnnotatedDatum_AnnotationType annotation_type);
...
void EncodeLocPrediction(const vector<LabelBBox>& all_loc_preds,
      ...
      const vector<vector<float> >& prior_variances,
      const MultiBoxLossParameter& multibox_loss_param,
      Dtype* loc_pred_data, Dtype* loc_gt_data, AnnotatedDatum_AnnotationType annotation_type) {
  int num = all_loc_preds.size();
  // CHECK_EQ(num, all_match_indices.size());
  // Get parameters.
  const CodeType code_type = multibox_loss_param.code_type();
    ...
    for (map<int, vector<int> >::const_iterator
      ...
      for (int j = 0; j < match_index.size(); ++j) {
        ...
        EncodeBBox(prior_bboxes[j], prior_variances[j], code_type,
                   encode_variance_in_target, gt_bbox, &gt_encode, annotation_type);
        if (annotation_type == AnnotatedDatum_AnnotationType_BBOX) {
          ...
          loc_gt_data[count * 8] = gt_encode.x_top_left();
          loc_gt_data[count * 8 + 1] = gt_encode.y_top_left();
          loc_gt_data[count * 8 + 2] = gt_encode.x_top_right();
          loc_gt_data[count * 8 + 3] = gt_encode.y_top_right();
          loc_gt_data[count * 8 + 4] = gt_encode.x_bottom_right();
          loc_gt_data[count * 8 + 5] = gt_encode.y_bottom_right();
          loc_gt_data[count * 8 + 6] = gt_encode.x_bottom_left();
          loc_gt_data[count * 8 + 7] = gt_encode.y_bottom_left();
        }
        ...
        if (bp_inside) {
          ...
          if (annotation_type == AnnotatedDatum_AnnotationType_BBOX) {
            ...
            loc_pred_data[count * 8] = loc_pred[j].x_top_left();
            loc_pred_data[count * 8 + 1] = loc_pred[j].y_top_left();
            loc_pred_data[count * 8 + 2] = loc_pred[j].x_top_right();
            loc_pred_data[count * 8 + 3] = loc_pred[j].y_top_right();
            loc_pred_data[count * 8 + 4] = loc_pred[j].x_bottom_right();
            loc_pred_data[count * 8 + 5] = loc_pred[j].y_bottom_right();
            loc_pred_data[count * 8 + 6] = loc_pred[j].x_bottom_left();
            loc_pred_data[count * 8 + 7] = loc_pred[j].y_bottom_left();
          }
        }
        if (encode_variance_in_target) {
          for (int k = 0; k < 4; ++k) {
...
template void EncodeLocPrediction(const vector<LabelBBox>& all_loc_preds,
      ...
      const vector<vector<float> >& prior_variances,
      const MultiBoxLossParameter& multibox_loss_param,
      float* loc_pred_data, float* loc_gt_data, AnnotatedDatum_AnnotationType annotation_type);
template void EncodeLocPrediction(const vector<LabelBBox>& all_loc_preds,
      const map<int, vector<NormalizedBBox> >& all_gt_bboxes,
      const vector<map<int, vector<int> > >& all_match_indices,
      const vector<NormalizedBBox>& prior_bboxes,
      const vector<vector<float> >& prior_variances,
      const MultiBoxLossParameter& multibox_loss_param,
      double* loc_pred_data, double* loc_gt_data, AnnotatedDatum_AnnotationType annotation_type);
...
void EncodeLocPrediction(const vector<LabelBBox>& all_loc_preds,
      ...
      Dtype* loc1_pred_data, Dtype* loc1_gt_data,
      AnnotatedDatum_AnnotationType annotation_type) {
  int num = all_loc_preds.size();
  // CHECK_EQ(num, all_match_indices.size());
  // Get parameters.
  const CodeType code_type = multibox_loss_param.code_type();
  ...
  for (int i = 0; i < num; ++i) {
    for (map<int, vector<int> >::const_iterator
      ...
      for (int j = 0; j < match_index.size(); ++j) {
        ...
        if (annotation_type == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING)
        {
          loc1_gt_data[count * 4 ] = gt_encode.x_offset();
          loc1_gt_data[count * 4 + 1] = gt_encode.y_offset();
          loc1_gt_data[count * 4 + 2] = gt_encode.width_ratio();
          loc1_gt_data[count * 4 + 3] = gt_encode.height_ratio();
        }
        // Store location prediction.
        CHECK_LT(j, loc_pred.size());
        if (bp_inside) {
          ...
          if (annotation_type == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING)
          {
            loc1_pred_data[count * 4 ] = loc_pred[j].x_offset();
            loc1_pred_data[count * 4 + 1] = loc_pred[j].y_offset();
            loc1_pred_data[count * 4 + 2] = loc_pred[j].width_ratio();
            loc1_pred_data[count * 4 + 3] = loc_pred[j].height_ratio();
          }
        }
        if (encode_variance_in_target) {
          for (int k = 0; k < 4; ++k) {
...
template void EncodeLocPrediction(const vector<LabelBBox>& all_loc_preds,
      ...
      float* loc1_pred_data, float* loc1_gt_data,
      AnnotatedDatum_AnnotationType annotation_type);
template void EncodeLocPrediction(const vector<LabelBBox>& all_loc_preds,
      const map<int, vector<NormalizedBBox> >& all_gt_bboxes,
      const vector<map<int, vector<int> > >& all_match_indices,
      ...
      double* loc1_pred_data, double* loc1_gt_data,
      AnnotatedDatum_AnnotationType annotation_type);
...
void GetDetectionResults(const Dtype* det_data, const int num_det,
      ...
      map<int, map<int, vector<NormalizedBBox> > >* all_detections, AnnotatedDatum_AnnotationType annotation_type) {
  ...
  for (int i = 0; i < num_det; ++i) {
    ...
    if (annotation_type == AnnotatedDatum_AnnotationType_BBOX) {
      ...
      start_idx = i * 16;
    }else if (annotation_type == AnnotatedDatum_AnnotationType_OFFSET_PRETRAIN||annotation_type == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING) {
      start_idx = i * 11;
    }
    ...
    if (annotation_type == AnnotatedDatum_AnnotationType_FOURPOINTS) {
      bbox.set_x_top_left(det_data[start_idx + 8]);
      bbox.set_y_top_left(det_data[start_idx + 9]);
      bbox.set_x_top_right(det_data[start_idx + 10]);
      bbox.set_y_top_right(det_data[start_idx + 11]);
      bbox.set_x_bottom_right(det_data[start_idx + 12]);
      bbox.set_y_bottom_right(det_data[start_idx + 13]);
      bbox.set_x_bottom_left(det_data[start_idx + 14]);
      bbox.set_y_bottom_left(det_data[start_idx + 15]);
    }
    if (annotation_type == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING)
    {
      bbox.set_x_offset(det_data[start_idx + 7]);
      bbox.set_y_offset(det_data[start_idx + 8]);
      bbox.set_width_ratio(det_data[start_idx + 9]);
      bbox.set_height_ratio(det_data[start_idx + 10]);
    }
    float bbox_size = BBoxSize(bbox);
    bbox.set_size(bbox_size);
