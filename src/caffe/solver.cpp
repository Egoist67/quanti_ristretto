...
#include "caffe/util/bbox_util.hpp"//added
...
void Solver<Dtype>::Init(const SolverParameter& param) {
  // CHECK(Caffe::root_solver() || root_solver_)
      // << "root_solver_ needs to be set for all non-root solvers";//added
...
void Solver<Dtype>::InitTrainNet() {
  ...
  // net_.reset(new Net<Dtype>(net_param));//src
  ///added
  if (Caffe::root_solver()) {
...
void Solver<Dtype>::InitTestNets() {
  ...
  for (int i = 0; i < num_test_net_instances; ++i) {
    ...
    // test_nets_[i].reset(new Net<Dtype>(net_params[i]));//src
    if (Caffe::root_solver()) {
...
//           << ", loss = " << smoothed_loss_;
//           // << " (" << per_s << " iter/s, " << lapse << "s/"//src
//           // << param_.display() << " iters), loss = " << smoothed_loss_;//src
//       iteration_timer_.Start();
...
template <typename Dtype>
void Solver<Dtype>::TestClassification(const int test_net_id) {//added
// void Solver<Dtype>::Test(const int test_net_id) {//src
  CHECK(Caffe::root_solver());
