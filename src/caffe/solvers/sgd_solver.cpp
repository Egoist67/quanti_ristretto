...
Dtype SGDSolver<Dtype>::GetLearningRate() {
  ...
  } else if (lr_policy == "plateau") {//added
...
void SGDSolver<Dtype>::PreSolve() {
  ...
  for (int i = 0; i < net_params.size(); ++i) {
   ...
   this->minimum_loss_ = std::numeric_limits<float>::max();//added
}
...
void SGDSolver<Dtype>::SnapshotSolverStateToBinaryProto(
  ...
  state.set_iter_last_event(this->iter_last_event_);//added
  state.set_minimum_loss(this->minimum_loss_);//added
  state.clear_history();
...
void SGDSolver<Dtype>::SnapshotSolverStateToHDF5(
  ...
  hdf5_save_int(file_hid, "iter_last_event", this->iter_last_event_);//src
  hdf5_save_float<Dtype>(file_hid, "minimum_loss", this->minimum_loss_);//src
...
void SGDSolver<Dtype>::RestoreSolverStateFromBinaryProto(
  ...
  this->iter_last_event_ = state.iter_last_event();//src
  this->minimum_loss_ = state.minimum_loss();//src
...
void SGDSolver<Dtype>::RestoreSolverStateFromHDF5(const string& state_file) {
  ...
  this->iter_last_event_ = hdf5_load_int(file_hid, "iter_last_event");//src
  this->minimum_loss_ = hdf5_load_float<Dtype>(file_hid, "minimum_loss");//src
