...
void DataTransformer<Dtype>::TransformAnnotation(
  ...
  if (anno_datum.type() == AnnotatedDatum_AnnotationType_BBOX || anno_datum.type() == AnnotatedDatum_AnnotationType_FOURPOINTS
     || anno_datum.type() == AnnotatedDatum_AnnotationType_OFFSET_PRETRAIN || anno_datum.type() == AnnotatedDatum_AnnotationType_OFFSET_FINETUNING) {
    ...
    for (int g = 0; g < anno_datum.annotation_group_size(); ++g) {
      ...
      for (int a = 0; a < anno_group.annotation_size(); ++a) {
        ...
        if (ProjectBBox(crop_bbox, resize_bbox, &proj_bbox, anno_datum.type())) {
