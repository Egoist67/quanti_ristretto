...
template <typename Dtype>
void LSTMRistrettoUnitLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  CUDA_POST_KERNEL_CHECK;
    if (this->phase_ == TEST) {
      ...
      this->QuantizeLayerOutputs_gpu(top[1]->mutable_gpu_data(),top[1]->count());
    }
}
...
template <typename Dtype>
void LSTMRistrettoUnitLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(LSTMRistrettoUnitLayer);
