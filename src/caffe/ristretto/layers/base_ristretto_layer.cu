...
__global__ void Trim2FixedPoint_kernel_purn(Dtype* data, const int cnt,
...
void BaseRistrettoLayer<Dtype>::QuantizeWeights_gpu(
  ...
  case QuantizationParameter_Precision_DYNAMIC_FIXED_POINT:
      Trim2FixedPoint_kernel_purn<<<CAFFE_GET_BLOCKS(cnt_weight), CAFFE_CUDA_NUM_THREADS>>>(
      weight, cnt_weight, bw_params_, rounding, fl_params_);
    //Trim2FixedPoint_gpu(weight, cnt_weight, bw_params_, rounding, fl_params_);
    if (bias_term) {
        Trim2FixedPoint_kernel_purn<<<CAFFE_GET_BLOCKS(weights_quantized[1]->count()), CAFFE_CUDA_NUM_THREADS>>>(
      weights_quantized[1]->mutable_gpu_data(), weights_quantized[1]->count(), bw_params_, rounding, fl_params_);
