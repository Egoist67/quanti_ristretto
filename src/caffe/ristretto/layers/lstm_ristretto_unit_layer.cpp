...
template <typename Dtype>
LSTMRistrettoUnitLayer<Dtype>::LSTMRistrettoUnitLayer(const LayerParameter& param)
...
template <typename Dtype>
void LSTMRistrettoUnitLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void LSTMRistrettoUnitLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  for (int n = 0; n < num; ++n) {
    ...
    if (this->phase_ == TEST) {
      ...
      this->QuantizeLayerOutputs_cpu(top[1]->mutable_cpu_data(),top[1]->count());
    }

}

template <typename Dtype>
void LSTMRistrettoUnitLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(LSTMRistrettoUnitLayer);
#endif

INSTANTIATE_CLASS(LSTMRistrettoUnitLayer);
REGISTER_LAYER_CLASS(LSTMRistrettoUnit);
