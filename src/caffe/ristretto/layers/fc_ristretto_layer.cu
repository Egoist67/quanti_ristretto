...
void FcRistrettoLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  //std::cout<<"fc_bias_before: "<<this->weights_quantized_[1]->mutable_cpu_data()[1]<<" "<<this->weights_quantized_[1]->mutable_cpu_data()[2]<<std::endl;

  this->QuantizeWeights_gpu(this->weights_quantized_, rounding,this->bias_term_);
  //std::cout<<"fc_bias_after: "<<this->weights_quantized_[1]->mutable_cpu_data()[1]<<" "<<this->weights_quantized_[1]->mutable_cpu_data()[2]<<std::endl;

  // Do forward propagation
  const Dtype* bottom_data = bottom[0]->gpu_data();
  Dtype* top_data = top[0]->mutable_gpu_data();
  const Dtype* weight = this->weights_quantized_[0]->gpu_data(); // source rhp
  //const Dtype* weight = this->blobs_[0]->gpu_data(); // rhp
  //std::cout<<"qqqqqq"<<this->M_<<std::endl;
  if (this->M_ == 1) {
