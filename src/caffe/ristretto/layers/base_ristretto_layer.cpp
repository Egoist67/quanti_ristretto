...
void BaseRistrettoLayer<Dtype>::QuantizeLayerInputs_cpu(Dtype* data,
      const int count) {
  Dtype* tmp_data = new Dtype[6];
  switch (precision_) {
    case QuantizationParameter_Precision_INTEGER_POWER_OF_2_WEIGHTS:
      break;
    case QuantizationParameter_Precision_DYNAMIC_FIXED_POINT:
      Trim2FixedPoint_cpu(data, tmp_data, count, bw_layer_in_, rounding_, fl_layer_in_);
      break;
    case QuantizationParameter_Precision_MINIFLOAT:
  ...
  //std::ofstream ofs_txt("/home/.prog_and_data/shared/codes/ezai/caffe_stand_alone_qmake/8_cpp.txt", std::ios::out | std::ios::app);

  //for(int i=0; i<6; ++i){
  //  ofs_txt<<tmp_data[i]<<std::endl;
  //}
  //ofs_txt.close();
  //delete []tmp_data;
}
...
template <typename Dtype>
void BaseRistrettoLayer<Dtype>::Trim2FixedPoint_cpu(Dtype* data, Dtype* tmp_data, const int cnt,
  ...
  for (int index = 0; index < cnt; ++index) {
    ...
    if(index==7){
      tmp_data[0] = max_data;
      tmp_data[1] = min_data;
      tmp_data[2] = data[index];
    }

    data[index] /= pow(2, -fl);

    if(index==7){
      tmp_data[3] = data[index];
    }
    ...
    if(index==7){
      tmp_data[4] = data[index];
    }
    data[index] *= pow(2, -fl);
    if(index==7){
      tmp_data[5] = data[index];
    }
  }
}
