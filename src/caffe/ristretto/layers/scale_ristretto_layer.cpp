...
ScaleRistrettoLayer<Dtype>::ScaleRistrettoLayer(
      const LayerParameter& param) : ScaleLayer<Dtype>(param),
...
void ScaleRistrettoLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (bottom.size() == 1 && this->blobs_.size() > 0) {
    ...
    ///////Quantize////////
    this->scale_quantized_.resize(2);
    this->scale_quantized_[0].reset(new Blob<Dtype>(scale_shape));
  }
  if (param.bias_term()) {
	...
	///////Quantize////////
    this->scale_quantized_.resize(this->bias_param_id_ + 1);
    this->scale_quantized_[this->bias_param_id_] = this->bias_layer_->blobs()[0];
  }
...
void ScaleRistrettoLayer<Dtype>::Forward_cpu(
  ...
  if(bottom.size() <= 1){
    ...
    this->QuantizeWeights_cpu(this->scale_quantized_, rounding,true);
  ...
  for (int n = 0; n < this->outer_dim_; ++n) {
    for (int d = 0; d < this->scale_dim_; ++d) {
      const Dtype factor = scale_quantized_data_[d];
